...
public class NotePlayer
{
	...
	private static boolean messageLoop(Scanner console)
	{
		...
		{
			// `song` has two sub-commands
			// `song save` will save the last-played song to a file
			// `song play` will play the requested song
			String[] args = input.split(" ");
			if (args[1].equals("save"))
			{
				// song save (name) (file)
				// (name) is the song's name; saved in the file
				// (file) is the archive file to put the song in
				// More than one song may be stored in a single
				// file, but song names must be unique within
				// that file. If a song is saved to a file with a
				// name that is already used in that file, it'll overwrite
				// the original with the new one.
				// `name` defaults to "mySong"
				// `file` defaults to "./songs"
				
				// syntax for archive file is a little silly but it
				// will work for Java's string parsing abilities
				// songs are saved as 
				// (song name)= (song content) 
				// with one song per line
				...
				saveToFile(file, name, LastInput);
			}
			...
			else if (args[1].equals("play"))
			{
				...
				// name is required, file is optional.
		...
		input = newInput;
		// save the processed input to make things easier when this is played back
		LastInput = input;
	...
	private static void playFromFile(String path, String name)
	{
		File archive = new File(path);
		
		try
		{
			Scanner fileReader = new Scanner(archive);
			while(fileReader.hasNextLine())
			{
				String song = fileReader.nextLine();
				if (song.startsWith(name + "= "))
				{
					// the first space plus one
					// although we don't really have to take off the first space
					// because Scanner is tolerant I think
					parseInput(song.substring(song.indexOf(" ") + 1));
					return;
				}
			}
		...
		{
			System.out.println("Archive file not found :/\nTry saving a song first!\n" + e.getMessage());
			return;
		}
		
	}
	
	private static void saveToFile(String path, String name, String song)
	{
		// look for the file
		File archive = new File(path);
		
		// use a Scanner to parse thru the file
		// and look for our song name
		// if it already exists, we'll have to delete
		// the line before we proceed
		ArrayList<String> lines = new ArrayList<String>();
		try 
		{
			Scanner fileReader = new Scanner(archive);
			// basically, load the file into memory
			// BUT if a given song is already named
			// what we want to use as our name,
			// don't load it
			while (fileReader.hasNextLine())
			{
		...
		{
			System.out.println("Song archive file not found: " + e.getMessage());
			System.out.println("Creating archive file...");
			lines.add("NotePlayer Song Archive v0.1");
			lines.add("Do NOT edit this file!");
			lines.add("Use the NotePlayer CLI to interact with archive files.");
		}
		
		lines.add(name + "= " + song + "\n");	
		
		// now, write the buffer to the file.
		// popular consensus demands using BufferedWriter for this
		try
		{
		...
		{
			System.out.println("Unhandled I/O exception! Make sure the file system is writable and that you have necessary permissons (rw).\n" + e.getMessage());
			return;
		}
